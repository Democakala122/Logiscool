...
def red_controll(keys_pressed, red):
    if keys_pressed[pygame.K_a] and red.x > 10:
        red.x -= VELOCITY
    if keys_pressed[pygame.K_d] and red.x < WIDTH // 2 - SPACESHIP_WIDTH - 5:
        red.x += VELOCITY
    if keys_pressed[pygame.K_w] and red.y > 5:
        red.y -= VELOCITY
    if keys_pressed[pygame.K_s] and red.y < HEIGHT - SPACESHIP_WIDTH - 5:
        red.y += VELOCITY
...
def yellow_controll(keys_pressed, yellow):
    ...
    if keys_pressed[pygame.K_UP] and yellow.y > 5 :
        yellow.y -= VELOCITY
    if keys_pressed[pygame.K_DOWN] and yellow.y < HEIGHT - SPACESHIP_WIDTH - 5:
        yellow.y += VELOCITY
...
def draw_frame(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    ...
    red_health_text = HEALTH_FONT.render(f"Health: {red_health}", True, BLACK)
    yellow_health_text = HEALTH_FONT.render(f"Health: {yellow_health}", True, BLACK)
...
def main():
    ...
    while gameOn:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(red_bullets) < MAXBULLETS:
                    ...
                    start_y = red.y + SPACESHIP_WIDTH // 2 - 2
                    bullet = pygame.Rect(start_x, start_y, 10, 5)
                    red_bullets.append(bullet)
                    LASER_SOUND.play()
                if event.key == pygame.K_RCTRL and len(yellow_bullets) < MAXBULLETS:
                    start_x = yellow.x - 5
                    start_y = yellow.y + SPACESHIP_WIDTH // 2 - 2
                    bullet = pygame.Rect(start_x, start_y, 10, 5)
        ...
        if red_health <= 0:
            break
            draw_winner("Yellow wins!")   # TODO MAKE draw_winner function
        if yellow_health <= 0:
            break
