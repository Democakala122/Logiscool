...
PATH = os.path.join(PATH, "kerekpar-adatok.txt")
...
with open(PATH ,encoding="utf-8",) as f:
    next(f)
    for line in f:
        line = line.strip().split(";")  # ['Kerék Express', 'Budapest', '5500']
        bolt = {"name": line[0],
                "city": line[1],
                "price": line[2]}
        boltok.append(bolt)
...
print(f"{boltok[max_index]['name']} a legdrágább üzlet, egy szervíz {boltok[max_index]['price']} Ft-be kerül")
...
for item in boltok:
    if item["name"] == bolt:
        talalat = True
        print(f"Ebben a boltban, 20%-os kedvezménnyel {akcio(item['price'])} Ft-be kerül")
        break
