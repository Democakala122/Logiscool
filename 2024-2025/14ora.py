...
#2. feladat: Olvassuk be ennyi eredményt, elsőzör a versenyző nevét, utána
# a versenyző elért pontszámát ( 0 - 100 ) egész szám
nevek = []
pontok = []
for i in range(1, n + 1):
    nevek.append(input(f"{i}. versenyző neve: "))
    pontok.append(int(input(f"{nevek[i-1]}. pontszáma:   ")))

#3. feladat: Határozzuk meg a versenyzők áltagos pontszámát
összeg = 0
for item in pontok:
    összeg += item
print("A pontszámok átlaga: ", round(összeg / n, 1))

#4. feladdat: Írjuk ki, hogy ki nyerte a versenyt és hogy hány pontot ért el
# (Ha több nyertes van, akkor mindet kiírjuk)
maxi = 0
for i in range(1, n):
...
pontszam = int(input("Adj meg egy pontszámot és megkeresem, hogy kik azok akik ettől rosszabbul teljesítettek: "))
for i in range(n):
